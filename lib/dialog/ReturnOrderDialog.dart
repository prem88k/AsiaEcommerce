...
class CallbackOfReturnOrderDialog {
...
class ReturnOrderDialog extends StatefulWidget {
  ...
  CallbackOfReturnOrderDialog callbackOfReturnOrderDialog;

//  VehicleList();
  ReturnOrderDialog(this.orderResponse,this.callbackOfReturnOrderDialog);


  @override
  ReturnOrderDialogState createState() => ReturnOrderDialogState();


}

class ReturnOrderDialogState extends State<ReturnOrderDialog> with TickerProviderStateMixin,ImagePickerListener  {
...
//      return Image(image: FileImage(_imagesList[0]));
  ...
  Future<void> getCancelOrder() async {
    ...
    var url = Uri.https(GlobalConfiguration().getString('url'),'/api/user/order-return-reasons');
...
//        var orderListResponse_obj = new OrderDetailResponse.fromJsonMap(data) as OrderDetailResponse;
  ...
  Future<void> CancelOrderSubmit() async {
    ...
    final String url = '${GlobalConfiguration().getString('base_url')}user/order/'+widget.orderResponse.order_id.toString()+'/return' ;
...
//      Toast.show(Api_constant.profile_update, context, gravity: Toast.CENTER);
      ...
      if(status ) {
        ...
        widget.callbackOfReturnOrderDialog.ReturnOrderCallback();
